...
def create_LSTM_batch(max_trk_len, fr_lists, id_lists, trainval, batch_sz = 32):
    ...
    while collected_num < batch_sz:
        ...
        recur_cnt = 0
        while len(id_lists[seq_idx][anchor_idx][1:]) <= min_len and recur_cnt <= 5:
            anchor_idx = random.choice([i for i in range(0, len(id_lists[seq_idx]))])
            recur_cnt += 1
        if recur_cnt > 5:
            print('while 1 stuck')
        ...
        anchor_fr_dets = fr_lists[seq_idx][int(anchor_det[0])-1][1:]
        ...
        anchor_img, is_valid1 = get_cropped_template(seq_name, anchor_det[0], anchor_det)
        neg_det[1:5] = augment_bbox(neg_det[1:5])
        neg_img, is_valid2 = get_cropped_template(seq_name, neg_det[0], neg_det[1:5])
        ...
        for pos_det in pos_dets:
            ...
            pos_img, is_valid = get_cropped_template(seq_name, pos_det[0], pos_det)
            if not is_valid:
                is_valid3 = False
                break
